"""R2 occupancy probe: count actual work-tiles (=CTAs) for the WIN config per shape@M, and sweep
smaller mma M-tiles (swap_ab=True) to see if MORE tiles -> more DRAM streams -> faster o_proj.
This directly tests the 'occupancy-bound at 4.62TB/s' hypothesis WITHOUT a new kernel.
Also: try mma N-tile variations to fill SMs. All via the production runner (tactic sweep, fine-grained)."""
...
def ntiles(M,N,mma,clu,swap):
    # kernel M-axis holds N (swap), N-axis holds M; tile counts
...
# focus o_proj M=1: sweep mma tiles to maximize tile count = DRAM streams
name,K_,N="o_proj",16384,7168
...
print(f"\no_proj M=1 cublaslt={cub:.2f}us  (target: beat via more tiles->more BW)",flush=True)
print(f"{'mma':>10} {'clu':>7} {'swap':>5} {'#tiles':>7} {'occ%':>5} {'us':>7} {'sp':>6} {'cos':>8}",flush=True)
res=[]
for mma in [(128,8),(128,16),(128,32),(128,64),(128,128),(256,64),(256,128),(256,256),(128,256)]:
    for clu in [(1,1),(2,1),(4,1),(1,2),(2,2),(1,4)]:
        swap=True
        if mma[0]==256 and clu[0]==1: continue  # 2cta needs cluster_m even? skip invalid
        try:
...
res.sort()
for t,mma,clu,nt,occ,cs in res[:12]:
    print(f"{str(mma):>10} {str(clu):>7} {'T':>5} {nt:>7} {occ:>4.0f}% {t:>6.2f} {cub/t:>5.2f}x {cs:>8.5f}",flush=True)
