"""R2 final canonical: WIN tactic vs cublaslt, all shapes x M, 15 windows x 80 replays (tight min), cos gate.
Confirms the o_proj plateau band and the q_a/moe_up/moe_down fixed-floor tie definitively."""
...
def cap_us(fn,windows=15,it=80):
    for _ in range(12): fn()
...
print(f"dev={torch.cuda.get_device_name(0)} WIN={WIN} (15x80 min)",flush=True)
print(f"{'shape':9} {'M':>3} {'cublaslt':>9} {'R2_WIN':>8} {'sp':>6} {'cos':>8} verdict",flush=True)
won=0;tot=0
for name,K_,N in shapes:
    ...
    for M in (1,4,16,64):
        ...
        win=cap_us(lambda: runner.forward(inputs,tactic=WIN)); sp=cub/win; tot+=1
        v="BEATS" if sp>1.02 else ("ties" if sp>0.98 else "loses")
        if sp>1.02: won+=1
        print(f"{name:9} {M:>3} {cub:>9.2f} {win:>8.2f} {sp:>5.2f}x {cs:>8.5f} {v}",flush=True)
print(f"\nSUMMARY: beats cublaslt on {won}/{tot}; ties rest; cos=1.0 throughout.",flush=True)
