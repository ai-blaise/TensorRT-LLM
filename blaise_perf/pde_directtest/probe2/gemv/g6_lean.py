"""g6: register-LEAN M=1 GEMV. Decode-and-accumulate inline (no 32-float arrays).
   Maximize occupancy. Sweep WPB. __launch_bounds__ to control regs. Focus small-N.
"""
...
__device__ __forceinline__ float e2m1f(unsigned v){
  unsigned e=(v>>1)&3u, m=v&1u, s=(v>>3)&1u, bits;
  if(e==0u) bits = m ? ((s<<31)|(126u<<23)) : (s<<31);
  else bits = (s<<31) | ((127u+e-1u)<<23) | (m<<22);
  return __int_as_float((int)bits);
}
// dot of 8 fp4 packed in wu vs xu (uint32 each), inline decode, returns sum
__device__ __forceinline__ float dot8(uint32_t wu, uint32_t xu){
  float s=0.f;
  #pragma unroll
  for(int i=0;i<8;i++){ int sh=i*4; s+=e2m1f((wu>>sh)&0xF)*e2m1f((xu>>sh)&0xF); }
  return s;
}
// M=1. one warp -> one n. lane strides over uint4 (2 blocks). decode-accumulate inline.
template<int WPB>
__global__ void __launch_bounds__(WPB*32) gemv1(
  ...
  for(int u=lane; u<nU; u+=32){
    ...
    // block blk = wv.x,wv.y (16 fp4); block blk+1 = wv.z,wv.w
    float p0=dot8(wv.x,xv.x)+dot8(wv.y,xv.y);
    float p1=dot8(wv.z,xv.z)+dot8(wv.w,xv.w);
...
'''
mod=load_inline(name="gemv_g6",cpp_sources=CPP,cuda_sources=CUDA,functions=["gemv"],
