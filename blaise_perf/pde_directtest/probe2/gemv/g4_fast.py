"""g4: warp-per-N-row GEMV with BRANCHLESS decode (no LUT) + uint4 vectorized loads.
   Each warp -> one n. Lane strides over K in 32-fp4 (16-byte=uint4) chunks. Coalesced.
"""
...
__device__ __forceinline__ float e2m1f(unsigned v){
  ...
  float base = (e==0u) ? (0.5f*(float)m) : ((1.0f+0.5f*(float)m)*(float)(1u<<(e-1)));
  return (v&8u)? -base : base;
}
// decode one uint32 (8 fp4 = 4 bytes? no: uint32=4 bytes=8 nibbles) into 8 floats
__device__ __forceinline__ void dec8(uint32_t w, float* o){
  #pragma unroll
  for(int i=0;i<8;i++) o[i]=e2m1f((w>>(i*4))&0xF);
}
template<int M, int WPB>
__global__ void gemv(const uint8_t* __restrict__ W, const uint8_t* __restrict__ Wsf,
  ...
  int Kb=K/2, Ks=K/16;
  const uint4* wrow=reinterpret_cast<const uint4*>(W+(size_t)n*Kb);  // each uint4=16B=32 fp4=2 blocks
  const __nv_fp8_e4m3* wsf=reinterpret_cast<const __nv_fp8_e4m3*>(Wsf)+(size_t)n*Ks;
  int nU=Kb/16;  // # uint4 per row = K/32 = Ks/2
  float acc[M];
  #pragma unroll
  for(int m=0;m<M;m++) acc[m]=0.f;
  for(int u=lane; u<nU; u+=32){
    uint4 wv=wrow[u];
    int blk=u*2;  // two 16-fp4 blocks: [wv.x,wv.y]=block blk, [wv.z,wv.w]=block blk+1
    float ws0=(float)wsf[blk], ws1=(float)wsf[blk+1];
    float wf[32];
    dec8(wv.x,wf); dec8(wv.y,wf+8); dec8(wv.z,wf+16); dec8(wv.w,wf+24);
    #pragma unroll
    for(int m=0;m<M;m++){
      ...
      float xf[32];
      dec8(xv.x,xf); dec8(xv.y,xf+8); dec8(xv.z,xf+16); dec8(xv.w,xf+24);
      float p0=0.f,p1=0.f;
      #pragma unroll
      for(int i=0;i<16;i++) p0+=wf[i]*xf[i];     // block blk (16 fp4)
      #pragma unroll
      for(int i=16;i<32;i++) p1+=wf[i]*xf[i];    // block blk+1
...
torch::Tensor gemv(torch::Tensor W, torch::Tensor Wsf, torch::Tensor X, torch::Tensor Xsf,
  ...
  const int WPB=8; int threads=WPB*32, blocks=(N+WPB-1)/WPB;
  ...
  if(M==1) gemv<1,WPB><<<blocks,threads,0,st>>>(wp,wsfp,xp,xsfp,a,N,K,yp);
  else if(M==4) gemv<4,WPB><<<blocks,threads,0,st>>>(wp,wsfp,xp,xsfp,a,N,K,yp);
  else if(M==8) gemv<8,WPB><<<blocks,threads,0,st>>>(wp,wsfp,xp,xsfp,a,N,K,yp);
  return Y;
}
'''
mod=load_inline(name="gemv_g4",cpp_sources=CPP,cuda_sources=CUDA,functions=["gemv"],
...
def main():
    ...
    print("ROW shape K N M backend cap_us bw% cos cutedsl_us",flush=True)
    for name,K,N in shapes:
        ...
        for M in (1,4,8):
            ...
            print(f"ROW {name} {K} {N} {M} mine {cu:.2f} {bw(N,K,M,cu):.1f} {c:.4f} {cl:.2f}",flush=True)
