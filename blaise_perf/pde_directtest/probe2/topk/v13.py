#!/usr/bin/env python3
# PDE top-k — ITER 11 (v13). v12: NPASS=2 MATCHES cute on block (8.2us, sm~0.99);
# final still 1.79x (18.5us) because the emit does 2 full-C scans (phase1 strictly-
# greater + phase2 equal-prefix). v13 keeps NPASS-byte approx radix but:
#   - phase-2 (equal-prefix fill) scans the COMPACTED survivor set s_act (boundary
#     bin, tiny) instead of full C -> removes one full-C scan from the emit.
#   - so emit = 1 full-C scan (phase1) + tiny s_act scan (phase2).
#   - total full-C touches: pass0 + phase1 = 2 (down from 3).
# Sweep NPASS in {2,3}. Also tries to keep s_act survivors == equal-prefix set so
# phase-2 reads them directly.
...
__global__ void __launch_bounds__(256) topk_v13(const float* __restrict__ scores,
  ...
  // emit. phase1: strictly-greater than resolved prefix (full-C, certain top-k).
  // phase2: equal-prefix fill from s_act survivors (tiny). s_act holds the pass0
  // boundary-bin; after NPASS passes the equal-prefix set is s_act filtered by pmaskS.
...
def build(npass):
    ...
    d=f"/tmp/torch_ext_pde13_{npass}"; os.environ["TORCH_EXTENSIONS_DIR"]=d; os.makedirs(d,exist_ok=True)
    src=CUDA_SRC_TMPL.replace("__NPASS__",str(npass)); t0=time.time()
    m=load_inline(name=f"pde_topk_v13_{npass}",cpp_sources="void launch_topk(torch::Tensor,torch::Tensor,torch::Tensor,int);",
...
def main():
    ...
    print(f"NPASS={NPASS}")
    print(f"{'case':>20} {'eC':>7} {'eV13':>7} {'capC':>8} {'capV13':>8} {'r':>8} {'sm':>6}",flush=True)
