#!/usr/bin/env python3
# PDE top-k — ITER 10 (v12). FLOOR FINDING: load-only = 4-6us captured; cute =
# 8-10us; my 8-pass radix = 12-25us. The 8 passes (fixed reset/reduce/walk/sync
# per pass) ARE the gap. The prod bar is set-match >= 0.99 (NOT exact). So use an
# APPROXIMATE few-pass radix:
#   NPASS-byte radix on the 32-bit score okey (NPASS in {2,3}), resolving the top
#   8*NPASS score bits, then emit {okey >= threshold_prefix} truncated to k. With
#   randn scores, NPASS=2 (top 16 bits) resolves the k-th score to within a few
#   ties -> set-match ~>=0.99. Active-set compaction after pass0 keeps later passes
#   tiny. Warp-private 8-hist + warp-walk (best structure from v8/v10).
# Sweep NPASS via argv: V12_NPASS=2 (default) or 3.
...
__device__ __forceinline__ void walk_warp(unsigned int* s_red, int krem, int lane,
  ...
  if(lane==0){ *out_digit=fd; *out_acc=(int)fa; }
}

// Approximate top-k: NPASS-byte MSD radix on 32-bit score okey, then emit okey>=thr.
__global__ void __launch_bounds__(256) topk_v12(const float* __restrict__ scores,
  ...
  // PASS 0 (top byte) full-C + compact
  {
    int sh=24;
  ...
  // APPROX threshold: accept okey >= prefixS (all bits below resolved prefix free).
  // This selects >= k elements (boundary-bin ties). Emit up to k (order-agnostic);
  // set-match counts intersection/k. To bias toward the true top-k, emit the
  // strictly-greater ones FIRST (definitely in top-k), then fill from boundary bin.
  unsigned int thr=prefixS;
  if(tid==0)s_fill=0u; __syncthreads();
  // phase 1: strictly greater than the resolved prefix (top NPASS bytes) -> certain top-k
  for(int c=tid;c<valid;c+=blockDim.x){ unsigned int key=s_key[c];
    if((key&pmaskS) > prefixS){ unsigned p=atomicAdd(&s_fill,1u); if(p<(unsigned)k)orow[p]=c; } }
  __syncthreads();
  // phase 2: equal-prefix (boundary bin) fill remaining slots
  if((int)s_fill < k){
    for(int c=tid;c<valid;c+=blockDim.x){ unsigned int key=s_key[c];
      if((key&pmaskS)==prefixS){ unsigned p=atomicAdd(&s_fill,1u); if(p<(unsigned)k)orow[p]=c; } }
  }
...
def build(npass):
    ...
    d=f"/tmp/torch_ext_pde12_{npass}"; os.environ["TORCH_EXTENSIONS_DIR"]=d; os.makedirs(d,exist_ok=True)
    src=CUDA_SRC_TMPL.replace("__NPASS__",str(npass))
    t0=time.time()
    m=load_inline(name=f"pde_topk_v12_{npass}",cpp_sources="void launch_topk(torch::Tensor,torch::Tensor,torch::Tensor,int);",
...
def main():
    ...
    print(f"NPASS={NPASS}")
    print(f"{'case':>20} {'eC':>7} {'eV12':>7} {'capC':>8} {'capV12':>8} {'r':>8} {'sm':>6}",flush=True)
