#!/usr/bin/env python3
# FINAL consolidated bench of the best kernel (v13 logic: NPASS-byte approx radix
# on fp32 score okey + parallel/warp threshold walk + active-set compaction +
# compacted phase-2 emit). Multi-seed set-match robustness + eager/captured timing
# vs cute_dsl at both prod shapes x B. NPASS via argv (default sweep 2 and 3).
import os, time, math, sys
import torch
import tensorrt_llm
NPASS_LIST=[2,3]
for a in sys.argv[1:]:
    if a.startswith("NPASS="): NPASS_LIST=[int(a.split("=")[1])]
CUDA_SRC_TMPL = r'''
...
__global__ void __launch_bounds__(256) topk_v(const float* __restrict__ scores,
...
def build(npass):
    ...
    d=f"/tmp/torch_ext_pdef_{npass}"; os.environ["TORCH_EXTENSIONS_DIR"]=d; os.makedirs(d,exist_ok=True)
    src=CUDA_SRC_TMPL.replace("__NPASS__",str(npass)); t0=time.time()
    m=load_inline(name=f"pde_topk_f_{npass}",cpp_sources="void launch_topk(torch::Tensor,torch::Tensor,torch::Tensor,int);",
...
def main():
    ...
    for NP in NPASS_LIST:
        m=build(NP)
        print(f"\n##### NPASS={NP} #####",flush=True)
        print(f"{'case':>20} {'capCute':>8} {'capMine':>8} {'speedup':>8} {'sm_mean':>8} {'sm_min':>8}",flush=True)
        for (C,k,label) in [(1032,64,"block C1032 k64"),(8192,1024,"final C8192 k1024")]:
            for B in [1,8,32,64]:
                ...
                for seed in range(6):
                    ...
                    op_=torch.full((B,k),-1,device=dev,dtype=torch.int32)
                    m.launch_topk(sc,sl,op_,k); torch.cuda.synchronize()
                    gold=torch.topk(sc.float(),k,dim=1).indices
                    sms.append(setmatch(op_,gold))
                print(f"{label:>16} B={B:>2} {capC:8.1f} {capV:8.1f} {capC/capV:7.2f}x {sum(sms)/len(sms):8.4f} {min(sms):8.4f}",flush=True)
