#!/usr/bin/env bash
# PDE G3 — build + run the device-resident DATA-DEPENDENT CONTROL FLOW
# (Indexer -> device top-k -> data-dependent gather) microbench.
...
#     /wt/blaise_perf/pde/build_run_g3.sh
# On the host, wrap with: flock -n /tmp/gpu001_lock_a timeout -s KILL 150 docker run ...
set -euo pipefail

WT="${WT:-/wt}"
SRC_DIR="$WT/cpp/tensorrt_llm/kernels/pde"
BENCH="$WT/blaise_perf/pde/pde_g3_bench.cu"
OUT="/tmp/pde_g3_bench"

echo "=== nvcc version ==="
nvcc --version | tail -2
echo "=== building PDE G3 device-control-flow bench (sm_100) ==="
