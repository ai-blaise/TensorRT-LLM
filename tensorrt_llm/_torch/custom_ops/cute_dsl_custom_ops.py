        ...
        def get_valid_tactics(
            ...
            **kwargs,
            ...
            for mma_tiler_mn, cluster_shape_mn, raster_along_m in itertools.product(
                ...
                # N=160 is only correct with a single N-CTA cluster (the
                # odd-tile TMEM shift is a 192-only special case) and
                # is_valid_mma_tiler_and_cluster_shape does NOT reject the
                # (160, cluster_n=2) combination — skip it here.
                if mma_tiler_mn[1] == 160 and cluster_shape_mn[1] != 1:
                    continue
                # Skip tactics where the cluster shape exceeds available
                # tiles. Launching more cluster CTAs than tiles causes
                # out-of-bounds memory access in the CuteDSL kernel.
