...
|---|---|---|---|---|
| **M3** | MoE EP comm → DeepEP | projected −1.48–1.57 ms/step (graphed *sizing*) | env flip | **NUANCED — two DeepEP modes:** (a) `DEEPEPLOWLATENCY` (what M3 specified) is **IBGDA-coupled by design** (`interface.py:105` "IBGDA is required") → on this no-IB single node it CPU-falls-back (`ibgda_nic_mem_gpu_map failed`) → e2e A/B NEUTRAL/jittery (50.3/52.9). (b) **`DEEPEP` (normal/intranode) uses NVLink P2P, NO IBGDA** (`interface.py:103`: IBGDA only for DeepEP's *internode* path) — the single-node-correct mode. **Testing `DEEPEP` now.** The −1.5ms was LL/IBGDA-specific, so DEEPEP-normal may beat NVLINK_TWO_SIDED or be a wash (both NVLink intranode) — measuring. DeepEP image built+works (`optrt-0680cd4082bc-deepep`). **RESULT: DEEPEP-normal = 44.6 tok/s/user (both rounds) = −14% vs NVLINK_TWO_SIDED (52) — a clear REGRESSION.** So the DeepEP investigation is CLOSED: (a) DeepEP-LL needs IB (CPU-fallback here → neutral), (b) DeepEP-normal runs clean on NVLink but is −14% slower than TRTLLM's purpose-built NVLINK_TWO_SIDED. **NVLINK_TWO_SIDED is the optimal a2a for this single-node no-IB B200.** M3's −1.5ms would require an IB-capable node (DeepEP-LL). |
