...
|---|---|---|---|---|
| **M3** | MoE EP comm → **DeepEP low-latency** | projected −1.48–1.57 ms/step (graphed *sizing*, not a live A/B) | `TRTLLM_FORCE_COMM_METHOD=DEEPEPLOWLATENCY` + `TRTLLM_DEEP_EP_TOKEN_LIMIT=64` | **e2e A/B = NEUTRAL/jittery (50.3 / 52.9 tok/s/user vs 51.6 baseline) — NO-GO on this hardware.** DeepEP-LL needs **IBGDA** (GPUDirect RDMA to an IB NIC); this is a no-IB single node (`NCCL_IB_DISABLE=1`) so it **CPU-falls-back** (decode log: `ibgda_nic_mem_gpu_map failed... CPU fallback path`, `cudaHostRegister IoMemory error=800`) → slower + adds jitter. The −1.5ms sizing assumed the GPU IBGDA path engages. **M3 would only win on an IB-capable node.** DeepEP image built + works (`optrt-0680cd4082bc-deepep`), so M3 is one env-flip away IF IB becomes available. |
