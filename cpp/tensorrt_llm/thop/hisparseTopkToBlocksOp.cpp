...
std::tuple<th::Tensor, th::Tensor, th::Tensor, th::Tensor> hisparseResolveBlocksToHostSlots(
    ...
    th::Tensor const& residentBlockFlags, th::Tensor const& residentRowStatus, th::Tensor const& requestIds,
    th::Tensor const& requestBlockHostSlots, th::Tensor const& requestBlockCommitGen,
    th::Tensor const& requestAdmitted)
{
    ...
    TORCH_CHECK(residentBlockFlags.is_cuda(), "resident_block_flags must be a CUDA tensor");
    TORCH_CHECK(residentRowStatus.is_cuda(), "resident_row_status must be a CUDA tensor");
    ...
    TORCH_CHECK(residentBlockFlags.scalar_type() == torch::kUInt8, "resident_block_flags must be uint8");
    TORCH_CHECK(residentRowStatus.scalar_type() == torch::kUInt8, "resident_row_status must be uint8");
    ...
    TORCH_CHECK(residentBlockFlags.dim() == 2, "resident_block_flags must have shape [rows, max_blocks_per_row]");
    TORCH_CHECK(residentRowStatus.dim() == 1, "resident_row_status must have shape [rows]");
    ...
    TORCH_CHECK(blockCounts.size(0) == rows, "block_counts rows mismatch: got ", blockCounts.size(0),
        ", expected ", rows);
    TORCH_CHECK(residentBlockFlags.size(0) == rows && residentBlockFlags.size(1) == maxBlocksPerRow,
        "resident_block_flags shape must match block_positions");
    TORCH_CHECK(residentRowStatus.size(0) == rows, "resident_row_status rows mismatch: got ",
        residentRowStatus.size(0), ", expected ", rows);
    ...
    TORCH_CHECK(residentBlockFlags.get_device() == blockPositions.get_device(),
        "resident_block_flags must be on the same CUDA device as block_positions");
    TORCH_CHECK(residentRowStatus.get_device() == blockPositions.get_device(),
        "resident_row_status must be on the same CUDA device as block_positions");
    ...
    auto residentFlags = residentBlockFlags.contiguous();
    auto residentStatus = residentRowStatus.contiguous();
    ...
    tk::invokeHisparseResolveBlocksToHostSlots(rowIds.data_ptr<int64_t>(), blocks.data_ptr<int32_t>(),
        counts.data_ptr<int32_t>(), residentFlags.data_ptr<uint8_t>(), residentStatus.data_ptr<uint8_t>(),
        tableIds.data_ptr<int64_t>(), tableHostSlots.data_ptr<int64_t>(), tableCommitGen.data_ptr<int64_t>(),
        tableAdmitted.data_ptr<bool>(), hostSlots.data_ptr<int64_t>(), commitGens.data_ptr<int64_t>(),
        blockStatus.data_ptr<uint8_t>(), rowStatus.data_ptr<uint8_t>(), static_cast<int32_t>(rows),
        static_cast<int32_t>(maxBlocksPerRow), static_cast<int32_t>(requestSlots),
...
std::tuple<th::Tensor, th::Tensor, th::Tensor, th::Tensor, th::Tensor, th::Tensor, th::Tensor> hisparsePlanHotSlots(
    ...
    th::Tensor const& residentBlockFlags, th::Tensor const& resolveRowStatus, th::Tensor const& hotHostSlot,
    th::Tensor const& hotCommitGen, th::Tensor const& hotLruTick, int64_t layerIdx, int64_t lruTickBase)
{
    ...
    TORCH_CHECK(blockCounts.size(0) == rows, "block_counts rows mismatch: got ", blockCounts.size(0),
        ", expected ", rows);
    TORCH_CHECK(residentBlockFlags.size(0) == rows && residentBlockFlags.size(1) == maxBlocksPerRow,
        "resident_block_flags shape must match host_slots");
    ...
    TORCH_CHECK(residentBlockFlags.get_device() == hostSlots.get_device(),
        "resident_block_flags must be on the same CUDA device as host_slots");
    ...
    tk::invokeHisparsePlanHotSlots(host.data_ptr<int64_t>(), gen.data_ptr<int64_t>(), counts.data_ptr<int32_t>(),
        residentFlags.data_ptr<uint8_t>(), resolveStatus.data_ptr<uint8_t>(), hotHost.data_ptr<int64_t>(),
        hotGen.data_ptr<int64_t>(), hotLru.data_ptr<int64_t>(), plannedHotSlots.data_ptr<int64_t>(),
        plannedLruTick.data_ptr<int64_t>(), missHostSlots.data_ptr<int64_t>(), missHotSlots.data_ptr<int64_t>(),
        missCounts.data_ptr<int32_t>(), hitFlags.data_ptr<uint8_t>(), rowStatus.data_ptr<uint8_t>(),
        static_cast<int32_t>(rows), static_cast<int32_t>(maxBlocksPerRow), static_cast<int32_t>(numLayers),
        static_cast<int32_t>(hotCapacity), static_cast<int32_t>(layerIdx), lruTickBase,
        at::cuda::getCurrentCUDAStream(host.get_device()).stream());
...
th::Tensor hisparseCommitHotSlots(th::Tensor const& hostSlots, th::Tensor const& commitGens,
    ...
    th::Tensor const& hotLruTick, th::Tensor const& residentBlockFlags, int64_t layerIdx)
{
    TORCH_CHECK(hostSlots.is_cuda(), "host_slots must be a CUDA tensor");
    TORCH_CHECK(commitGens.is_cuda(), "commit_gens must be a CUDA tensor");
    ...
    tk::invokeHisparseCommitHotSlots(host.data_ptr<int64_t>(), gen.data_ptr<int64_t>(),
        ...
        hotLruTick.data_ptr<int64_t>(), residentFlags.data_ptr<uint8_t>(), rowStatus.data_ptr<uint8_t>(),
        static_cast<int32_t>(rows), static_cast<int32_t>(maxBlocksPerRow), static_cast<int32_t>(numLayers),
        static_cast<int32_t>(hotCapacity), static_cast<int32_t>(layerIdx),
        at::cuda::getCurrentCUDAStream(host.get_device()).stream());
    return rowStatus;
}
...
std::tuple<th::Tensor, th::Tensor> hisparseBuildHotIndices(th::Tensor const& topkIndices,
    ...
    th::Tensor const& commitRowStatus, th::Tensor const& residentBlockFlags, int64_t hotCapacity,
    int64_t tokensPerBlock, int64_t strideFactor, int64_t layerIdx)
{
    TORCH_CHECK(topkIndices.is_cuda(), "topk_indices must be a CUDA tensor");
    TORCH_CHECK(blockPositions.is_cuda(), "block_positions must be a CUDA tensor");
    TORCH_CHECK(plannedHotSlots.is_cuda(), "planned_hot_slots must be a CUDA tensor");
    ...
    TORCH_CHECK(residentBlockFlags.get_device() == topkIndices.get_device(),
        "resident_block_flags must be on the same CUDA device as topk_indices");
    ...
    tk::invokeHisparseBuildHotIndices(topk.data_ptr<int32_t>(), blocks.data_ptr<int32_t>(),
        ...
        residentFlags.data_ptr<uint8_t>(), hotIndices.data_ptr<int32_t>(), rowStatus.data_ptr<uint8_t>(),
        static_cast<int32_t>(rows), static_cast<int32_t>(indexTopK), static_cast<int32_t>(maxBlocksPerRow),
        static_cast<int32_t>(hotCapacity), static_cast<int32_t>(tokensPerBlock),
        static_cast<int32_t>(strideFactor), static_cast<int32_t>(layerIdx),
...
TORCH_LIBRARY_FRAGMENT(trtllm, m)
{
    ...
    m.def(
        ...
        "Tensor resident_block_flags, Tensor resident_row_status, Tensor request_ids, Tensor request_block_host_slots, "
        "Tensor request_block_commit_gen, Tensor request_admitted) -> (Tensor, Tensor, Tensor, Tensor)");
    m.def(
    ...
    m.def(
        ...
        "Tensor resident_block_flags, Tensor resolve_row_status, Tensor hot_host_slot, Tensor hot_commit_gen, "
        "Tensor hot_lru_tick, "
    ...
    m.def(
        ...
        "Tensor(b!) hot_commit_gen, Tensor(c!) hot_lru_tick, Tensor resident_block_flags, int layer_idx) -> Tensor");
    m.def(
        "hisparse_build_hot_indices(Tensor topk_indices, Tensor block_positions, Tensor planned_hot_slots, "
        "Tensor block_counts, Tensor commit_row_status, Tensor resident_block_flags, int hot_capacity, "
        "int tokens_per_block, int stride_factor, int layer_idx) -> (Tensor, Tensor)");
}
