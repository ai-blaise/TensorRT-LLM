...
std::tuple<at::Tensor, at::Tensor> fused_sigmoid_mul_quant_nvfp4_swizzled(
    at::Tensor const& input, at::Tensor const& gate, at::Tensor const& sf_scale, int64_t sf_vec_size)
{
    CHECK_TH_CUDA(input);
    CHECK_TH_CUDA(gate);
    CHECK_CONTIGUOUS(input);
    ...
    auto const& inputShape = input.sizes();
    auto const& gateShape = gate.sizes();
    TORCH_CHECK(inputShape.size() == 2, "input should be 2D tensor [M, N].");
    TORCH_CHECK(gateShape.size() == 2, "gate should be 2D tensor [M, N].");
    TORCH_CHECK(inputShape == gateShape, "input and gate shapes must match.");
    TORCH_CHECK(input.scalar_type() == gate.scalar_type(), "input and gate dtypes must match.");
    ...
    if (input.scalar_type() == at::ScalarType::Half)
    {
        kernels::invokeFusedSigmoidMulQuantize<half>(reinterpret_cast<half const*>(input.data_ptr()),
            reinterpret_cast<half const*>(gate.data_ptr()), sfScalePtr, output_fp4.data_ptr<uint8_t>(),
            output_sf.data_ptr<uint8_t>(), static_cast<int>(m), static_cast<int>(n), static_cast<int>(sf_vec_size),
            stream);
    }
    else if (input.scalar_type() == at::ScalarType::BFloat16)
    {
#ifdef ENABLE_BF16
        kernels::invokeFusedSigmoidMulQuantize<__nv_bfloat16>(reinterpret_cast<__nv_bfloat16 const*>(input.data_ptr()),
            reinterpret_cast<__nv_bfloat16 const*>(gate.data_ptr()), sfScalePtr, output_fp4.data_ptr<uint8_t>(),
            output_sf.data_ptr<uint8_t>(), static_cast<int>(m), static_cast<int>(n), static_cast<int>(sf_vec_size),
            stream);
#else
        C10_THROW_ERROR(NotImplementedError, "BFloat16 not enabled.");
    ...
    {
        C10_THROW_ERROR(NotImplementedError, "fused_sigmoid_mul_quant_nvfp4_swizzled only supports fp16/bf16.");
    }
...
TORCH_LIBRARY_FRAGMENT(trtllm, m)
{
    ...
    m.def(
        "fused_sigmoid_mul_quant_nvfp4_swizzled(Tensor input, Tensor gate, Tensor sf_scale, int sf_vec_size=16) -> "
        "(Tensor, Tensor)");
}
...
TORCH_LIBRARY_IMPL(trtllm, CUDA, m)
{
    ...
    m.impl("fused_sigmoid_mul_quant_nvfp4_swizzled", &tensorrt_llm::torch_ext::fused_sigmoid_mul_quant_nvfp4_swizzled);
}
