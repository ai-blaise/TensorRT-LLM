"""Anti-reward-hack matched-shape bench: vanilla native TRTLLMGen MoE local compute
vs fused WarpDecode, IDENTICAL per-rank (tokens, experts, precision), both graph+PDL.
Per-rank shape derived from 32-user EP=G deployment: experts=128/G, tokens=conc*8/G.
Single GPU = per-rank local compute (EP dispatch identical for both, excluded)."""
...
NE_GLOBAL, TK, NG, TG, SV, DSR = 128, 8, 8, 4, 16, 2
os.environ.setdefault("TRTLLM_ENABLE_PDL","1")
dev=torch.device("cuda")
CONTEXTS=[1024,8192,16384,32768,65536,102400,131072]

def t(fn, it=80, rp=5, warm=15):
...
def run_shape(LE, ntok):
    ...
    runner=FP4BlockScaleMoERunner(NE_GLOBAL,TK,NG,TG,INTERMEDIATE,0,LE,None,DSR,True,ActType_TrtllmGen.SwiGlu.value,tune_max_num_tokens=8192,use_dp=False)
    # pick a valid c-bucket tactic by nearest token bucket
    ...
    meta=torch.ops.trtllm.moe_sort(token_selected_experts=ids,token_final_scales=wf32,num_experts=NE_GLOBAL,top_k=TK,local_expert_offset=0,local_num_experts=LE,tile_tokens_dim=128)
    ...
    def warpdecode():
        h,hsf=torch.ops.trtllm.cute_dsl_nvfp4_gather_grouped_gemm_act_fusion_blackwell(
            ...
            permuted_idx_to_expanded_idx=p2e,num_non_exiting_tiles=nt,global_sf=gsf,num_experts=NE_GLOBAL,top_k=TK,
        ...
        torch.ops.trtllm.cute_dsl_nvfp4_grouped_gemm_finalize_inplace_blackwell(
            ...
            permuted_idx_to_expanded_idx=p2e,num_non_exiting_tiles=nt,token_final_scales=wf32,num_experts=NE_GLOBAL,top_k=TK,
            num_local_experts=LE,local_expert_offset=0,tile_size=128,output_dtype=torch.bfloat16)
        return out
    nat=t(native); wd=t(warpdecode)
    del w13,w13sf,w2,w2sf,runner; torch.cuda.empty_cache()
    return nat, wd

rows=[]
print(f"{'GPUs':>4} {'conc':>4} {'experts/rank':>12} {'tok/rank':>9} {'native_us':>10} {'warpdec_us':>11} {'speedup':>8}",flush=True)
for G in [2,4,8]:
    LE=NE_GLOBAL//G
    for conc in [16,32]:
        ntok=max(conc*TK//G,1)
        nat,wd=run_shape(LE, ntok)
        rows.append({"G":G,"conc":conc,"LE":LE,"ntok":ntok,"native_us":nat,"wd_us":wd,"speedup":nat/wd})
        print(f"{G:>4} {conc:>4} {LE:>12} {ntok:>9} {nat:>9.2f} {wd:>10.2f} {nat/wd:>7.3f}x",flush=True)
import pathlib; pathlib.Path('/home/spencer/work/benches/artifacts').mkdir(parents=True,exist_ok=True)
pathlib.Path('/home/spencer/work/benches/artifacts/matched_shape.json').write_text(json.dumps(rows,indent=2))
print("\n(MoE decode is context-independent: 1 token/user/step regardless of context. The above per-(GPU,conc) local-compute ratio holds across all contexts 1k-128k.)",flush=True)
