...
def dn_k(mW2, mInter, mSFW2, mEids, mRW, mOut, NW: cutlass.Constexpr, TOPK: cutlass.Constexpr):
    ...
    h=bidx*WARPS+warp                       # output dimension
    t=bidz                                  # token
    acc=cutlass.Float32(0.0)
    for ks in cutlass.range_constexpr(TOPK):
        pair=t*TOPK+ks; e=mEids[pair]; rw=cutlass.Float32(mRW[pair])
        w=lane; part=cutlass.Float32(0.0)
        while w < NW:
            dw=TensorSSA(cute.arch.cvt_f4e2m1x8_to_f16x8(mW2[e,h,w].ir_value()),(8,),cutlass.Float16)
            sfd=cutlass.Float32(mSFW2[e,h,w//2])
            s=cutlass.Float32(0.0)
            for i in cutlass.range_constexpr(8):
                s=s+cutlass.Float32(dw[i])*cutlass.Float32(mInter[pair,w*8+i])
            part=part+s*sfd
            w=w+32
        acc=acc+part*rw
    o=cute.arch.warp_reduction_sum(acc)
    if lane==0:
        mOut[t,h]=o.to(cutlass.BFloat16)

@cute.jit
def launch(mW2,mInter,mSFW2_raw,mEids,mRW,mOut, E:cutlass.Constexpr,H:cutlass.Constexpr,INTER:cutlass.Constexpr,T:cutlass.Constexpr,LP:cutlass.Constexpr,TOPK:cutlass.Constexpr, stream):
...
E,H,INTER,T,TOPK = 16,7168,2048,32,8; LP=T*TOPK; NW=INTER//8; sfk=INTER//16
...
Inter=(torch.randn(LP,INTER,device=dev,dtype=torch.bfloat16)*0.1)
...
mW2=from_dlpack(W2u); mInter=from_dlpack(Inter); mSFW2=from_dlpack(SFW2.view(torch.float8_e4m3fn)); mEids=from_dlpack(eids); mRW=from_dlpack(rw); mOut=from_dlpack(Out)
cur=cuda.CUstream(torch.cuda.current_stream().cuda_stream)
comp=cute.compile(launch,mW2,mInter,mSFW2,mEids,mRW,mOut,E,H,INTER,T,LP,TOPK,cur)
comp(mW2,mInter,mSFW2,mEids,mRW,mOut,cur); torch.cuda.synchronize()
...
W2f=deq32(W2u.reshape(E*H,NW),SFW2.reshape(E*H,sfk),INTER).reshape(E,H,INTER); If=Inter.float()
ref=torch.zeros(T,H,device=dev)
for l in range(LP):
    t=l//TOPK; e=eids[l]; ref[t]+= (W2f[e]@If[l])*rw[l].item()
cos=torch.nn.functional.cosine_similarity(Out.float().flatten().unsqueeze(0),ref.flatten().unsqueeze(0)).item()
print(f"DN-FAITHFUL cosine={cos:.6f} -> {'PASS' if cos>0.999 else 'FAIL'}")
def bench(fn,it=50,wu=10):
    for _ in range(wu): fn()
    torch.cuda.synchronize(); s=torch.cuda.Event(True); e=torch.cuda.Event(True); s.record()
    for _ in range(it): fn()
    e.record(); torch.cuda.synchronize(); return s.elapsed_time(e)/it*1000
t=bench(lambda: comp(mW2,mInter,mSFW2,mEids,mRW,mOut,cur)); wb=LP*(H*INTER//2)
print(f"DN-FAITHFUL E={E} LP={LP}: {t:.2f}us  weight-BW={wb/(t*1e-6)/1e12:.2f}TB/s")
