"""WarpDecode Round 5: apply the R4 lever (F16 within-block + tree reduction) to the DOWN kernel (FC2).
Baseline down_v3 (HPW=8, 3.09 TB/s) does per-word `s += prod[i]` serial F32 reduction. R5: prod stays
F16, tree-reduce 8->1 (depth 3), F32 accumulate across words/experts. CZS = cosine vs FP32 ref."""
    ...
    for ks in cutlass.range_constexpr(TOPK):
        ...
        while w < NW:
            ...
            for q in cutlass.range_constexpr(HPW):
                ...
                prod = dw * iv
                p01 = prod[0] + prod[1]; p23 = prod[2] + prod[3]; p45 = prod[4] + prod[5]; p67 = prod[6] + prod[7]
                sF = (p01 + p23) + (p45 + p67)
                parts[q] = parts[q] + sF.to(cutlass.Float32) * sfd
            w = w + 32
...
for l in range(LP):
    t = l // TOPK; e = eids[l]; ref[t] += rw[l].float() * (W2f[e] @ If[l])
cos = torch.nn.functional.cosine_similarity(Out.float().flatten().unsqueeze(0), ref.float().flatten().unsqueeze(0)).item()
def bench(it=80, wu=20):
    f = lambda: comp(mW2, mI3, mSFW2, mEids, mRW, mOut, cur)
    for _ in range(wu): f()
    torch.cuda.synchronize(); s = torch.cuda.Event(True); e = torch.cuda.Event(True); s.record()
    for _ in range(it): f()
    e.record(); torch.cuda.synchronize(); return s.elapsed_time(e) / it * 1000
t_us = bench(); wb = LP * (H * INTER // 2)
print(f"R5 F16-tree DOWN (B={T}): cos={cos:.5f} {t_us:.1f}us {wb/(t_us*1e-6)/1e12:.2f}TB/s = {wb/(t_us*1e-6)/1e12/6.8*100:.0f}% peak (down_v3 baseline 3.09)", flush=True)
