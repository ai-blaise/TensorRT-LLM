...
def test_r20_manifest_has_no_helix_or_smc_fallback():
    ...
    assert manifest.count("value: UCX") >= 2
    assert "value: LIBFABRIC" not in manifest
...
def test_r20_nixl_gate_readiness_audit_is_read_only_and_fail_closed():
    ...
    assert 'EXPECTED_NIXL_PLUGIN_BACKEND="${EXPECTED_NIXL_PLUGIN_BACKEND:-UCX}"' in script
...
def test_r20_nixl_plugin_matrix_probe_is_no_traffic_and_checks_cleanup():
    ...
    assert "immediate pre-A/B peer-KV gate is the NIXL UCX plugin" in readme
