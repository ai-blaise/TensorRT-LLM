...
def _worker_m5g(rank, world_size, master_port, policy, num_layers,
                tmpdir_path):
    """Child-process entry point for the M5g FUSED dual-cache broadcast
    test (one NCCL broadcast per layer covering BOTH indexer-K and
    dense KV via maybe_broadcast_active_blocks_fused).
    """
    os.environ.setdefault("NCCL_NVLS_ENABLE", "0")
    os.environ["MASTER_ADDR"] = "127.0.0.1"
    os.environ["MASTER_PORT"] = str(master_port)
    ...
    try:
        ...
        for layer_idx in range(num_layers):
            ...
            # ONE fused broadcast covering both caches
            state.maybe_broadcast_active_blocks_fused(
                layer_idx=layer_idx,
                cache_slots=(indexer_pool, dense_pool),
            ...
            for slot, blk_id in enumerate(active_block_ids.tolist()):
                ...
                if indexer_pool[blk_id, 0].item() != indexer_expected:
                    errors.append(
                        f"M5g indexer L{layer_idx} blk{blk_id} rank{rank}: "
                        f"got {indexer_pool[blk_id, 0].item()}, "
                        f"exp {indexer_expected}")
                    break
                if dense_pool[blk_id, 0].item() != dense_expected:
                    errors.append(
                        f"M5g dense L{layer_idx} blk{blk_id} rank{rank}: "
                        f"got {dense_pool[blk_id, 0].item()}, "
                        f"exp {dense_expected}")
                    break

            if not (indexer_pool[inactive_mask]
                     == (rank + 1) * 11).all().item():
                errors.append(
                    f"M5g L{layer_idx} rank{rank}: indexer inactive "
                    f"corrupted")
            if not (dense_pool[inactive_mask] == (rank + 1) * 13).all().item():
                errors.append(
                    f"M5g L{layer_idx} rank{rank}: dense inactive "
                    f"corrupted")
        ...
        with open(result_path, "w") as f:
            if errors:
                ...
                f.write(
                    f"PASS rank={rank} layers={num_layers} fused_dual=ok\n")
    finally:
        dist.destroy_process_group()
...
@pytest.mark.skipif(not HAS_CUDA,
                    reason="needs >=2 CUDA devices for multi-rank NCCL test")
@pytest.mark.parametrize("policy", ["round_robin", "contiguous"])
def test_real_nccl_m5g_fused_dual_cache_broadcast(tmp_path, policy):
    """End-to-end NCCL test of M5g: ONE fused broadcast per layer
    covering both indexer-K and dense KV (system-level launch-overhead
    fusion)."""
    import torch.multiprocessing as mp

    world_size = 2
    num_layers = 8
    master_port = 29590 + (0 if policy == "round_robin" else 1)
