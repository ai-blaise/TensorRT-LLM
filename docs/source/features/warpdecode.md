...
The selected NVFP4 crossover is graph-safe and PDL-positive. A 2026-06-02 B200 capture/replay sweep over c1/c2/c4/c8/c16/c32 and route patterns `round_robin`, `slot8`, `paired`, and `single_expert` matched the explicit TRTLLMGen runner exactly (`max_abs=0`). With CUDA graph replay, `TRTLLM_ENABLE_PDL=1` beat PDL-disabled replay in every tested pattern and bucket. For the production-like `round_robin` pattern, PDL-on graph replay was faster by 1.122x, 1.079x, 1.064x, 1.075x, 1.081x, and 1.051x for c1 through c32. This keeps the current `TRTLLM_ENABLE_PDL=1` default and avoids adding a runtime toggle. Artifacts: `artifacts/warpdecode/pdl_confirm_20260602/`, `artifacts/warpdecode/cuda_graph_confirm_20260602.json`, and `artifacts/warpdecode/cuda_graph_pdl_summary_20260602.json`.

The next optimization target remains a true output-warp or hybrid CuTe/CZS NVFP4 kernel that consumes post-EPLB and post-dispatch top-k slot metadata directly, aligns with c1/c2/c4/c8/c16/c32 CUDA-graph buckets, and avoids expert padding, generic permutation/finalize, and full intermediate buffers. The NVFP4 guard intentionally allows external communication and EPLB slot ids because the scheduler hook runs after dispatch/remap; the BF16 compatibility path remains conservative. IKP on the direct 32-token path showed FC1/SwiGLU and FC2 BMM dominate runtime while routing/finalize are small, so the production path must change the compute and padding structure rather than only replacing the final scatter/combine stage. Route values are not cached across decode steps; only stable per-layer/per-bucket route and scratch buffer addresses are candidates for graph replay.
...
A 2026-06-02 route-local tactic sweep found valid but non-promotable scheduling wins. Tactics such as `[16, 52]` and `[32, 52]` improved highly localized `slot8` patterns, but they regressed or failed to materially improve `round_robin` and `paired` patterns that better approximate production routing pressure. A dynamic Python selector based on `torch.unique(topk_ids)` was also rejected because selector overhead erased the win. Route-locality remains a systems-level opportunity only if the scheduler can provide cheap route-shape metadata before runner selection, or if the final Cursor op consumes the routes directly. Artifact: `artifacts/warpdecode/static_tactic_confirm_c16_c32_20260602.json`.

A CUTLASS FP4 GEMV direct-slot floor was rejected as a replacement for the current bridge. The standalone Blackwell example compiled for `sm_100a`, but benchmark-only FC1/FC2 target-shape probes measured about 0.0616 ms for FC1-like c32 and 0.0740 ms for FC2-like c32 before SwiGLU, top-k accumulation, routing, or launch overhead. That lower bound is already slower than the current full bridge at roughly 0.071-0.072 ms for c32, so a direct CUTLASS GEMV wrapper is not a credible promotion path. Artifact: `artifacts/warpdecode/cutlass_fp4_gemv_floor_20260602/`.
