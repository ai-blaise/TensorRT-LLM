...
| M5d       | `850c09b6`   | First real indexer-K owner broadcast. Superseded by active-block broadcast because full-slot broadcast wastes bandwidth at long context. |
| M5e       | `7fc8b1d0`   | Active-block-only indexer-K broadcast. Computes the current step's touched block ids, gathers those rows, broadcasts only that payload, and scatters back. This is the production indexer-K broadcast shape. |
| M5f       | `d0a15676`   | Dense KV broadcast alongside indexer-K using the same active-block id set. This matches the z.ai "both caches" pattern while avoiding full-pool wire traffic. |
| M5g       | `6f40714b`   | Fused indexer-K+dense-KV single-broadcast path explored. Measured slower at production scale because pack/split overhead exceeded the saved NCCL launch; retained as an optional helper only. |
| M5d-tight | `dbdf9350`   | Owner-local per-rank memory savings via layer masks plus non-owner scratch buffers. |
| M5d-tight-v2 | `fde2b78d` | Balanced layer-mask sums across CP ranks so every rank gets identical `num_blocks`; this fixes block-id consistency for the active-block broadcast with 61-layer models. |

## Current production state

The merged production path is M5e + M5f + M5d-tight-v2:

- LayerSplit is enabled only through runtime `SparseAttentionConfig.layersplit_*` fields; HF model cards do not enable it.
- Each CP rank owns a subset of layers under `round_robin` or `contiguous` ownership.
- `_create_kv_cache_manager` installs a balanced owner-local `layer_mask` when LayerSplit is enabled and CP > 1. The mask pads phantom layers so all CP ranks allocate the same number of blocks, preserving global block-id consistency even when the real layer count is not divisible by CP size.
- Non-owner layers read from per-rank scratch buffers through the normal `get_indexer_k_cache_buffers()` and `get_buffers()` accessors, so downstream DSA kernels do not need a separate LayerSplit source path.
- Before DSA indexer compute, `Indexer.forward` computes active block ids from `kv_lens`, `seq_lens`, and `block_table`, then broadcasts only those blocks for both indexer-K and dense KV.
- Broadcast source rank is translated from CP-local owner rank to the global rank in `mapping.cp_group`. This is required for production TP/CP meshes where the CP group can be `[2, 3]`, `[4, 5]`, etc.
- System fusion remains available through `maybe_broadcast_active_blocks_fused`, but production uses separate indexer-K and dense-KV broadcasts because the measured fused path was slower at production scale.

## Remaining work

| Area | Next step |
|------|-----------|
| Full serving validation | Run the 32-concurrent 1k-128k matrix with LayerSplit enabled in the chosen op-trt deployment once the system-level benchmark phase resumes. |
| Partial-rank transfer | Keep `layersplit_all_cp_ranks_transfer=True` until a correct partial-rank protocol is implemented and validated. |
| Kernel path | The stage-for-broadcast CuTe DSL kernel is CZS-proved and byte-exact, but the current production active-block path is PyTorch gather/broadcast/scatter. Replace the gather/scatter pieces with fused CuTe/CZS kernels only if IKP shows they dominate at production shapes. |
| Fusion / overlap | Revisit `maybe_broadcast_active_blocks_fused` or cross-layer overlap only when IKP/system traces show NCCL launch overhead or visible broadcast wait, since prior B200 sweeps showed sync active-block transfer wins at current production sizes. |
...
**Production default is synchronous active-block transfer.** The old configurable `layersplit_broadcast_mode` and payload-size knobs are intentionally removed from production config; overlap/fusion modes are retained as benchmarked helpers, not default runtime policy.

Run the benchmark with:

```bash
NCCL_NVLS_ENABLE=0 CUDA_VISIBLE_DEVICES=<gpu_a>,<gpu_b> \
    python3 tests/unittest/_torch/run_bench_layersplit_overlap.py
```
...
| Owner-of-layer-L broadcasts its KV cache to peer CP ranks before that layer's attention compute (§4 Fig 4(b)).        | `dsa.py` Indexer.forward hook calls `LayerSplitRuntimeState.maybe_broadcast_active_blocks(layer_idx, cache_slot, active_block_ids, cp_group)` synchronously before `pre_indexer_proj`. NCCL `dist.broadcast(src=global_owner_rank, group=cp_group_pg)` publishes the owner's bytes; `global_owner_rank` is translated from CP-local ownership through `mapping.cp_group`.            | **DONE** — verified end-to-end via `_worker_m5e` test on CP=2/4, plus source-rank translation unit coverage. |
...
| Indexer-cache size is ~1/8 of dense KV cache (blog §4).                                                               | `LayerSplitRuntimeState.ensure_heartbeat_payload(..., channel="indexer")` historically halved the indexer payload to ~1/8 of the kv payload; today the bench scaffold respects the ratio via the `payload_bytes // 8` split in the channel branch.                                       | Scaffold present (M9 bench respects the ratio). M5f (current commit) ships both broadcasts per the blog. |
| Broadcast BOTH the indexer-K cache AND the dense KV cache before attention (blog §4 Fig 4(b)).                       | M5f extends the dsa.py Indexer.forward hook: after the M5e indexer-K active-block broadcast, the hook reads the dense KV pool via `kv_cache_manager.get_buffers(layer_idx)` (NHD layout), flattens to `(num_blocks, K)`, and runs the SAME `maybe_broadcast_active_blocks` helper on it with the SAME `active_block_ids`. One block-id computation drives both broadcasts. | **DONE** — verified end-to-end via the new `_worker_m5f` test on CP=2 and CP=4 (the test mocks BOTH a small indexer-K pool and a larger dense-KV pool, owner stamps distinguishable bytes into both, every rank verifies (a) both pools' active blocks now hold owner's bytes (b) both pools' inactive blocks were not touched (c) no cross-contamination between the two broadcasts). |
