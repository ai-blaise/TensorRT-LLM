...
DSA Indexer, FP4 indexer K cache, CuTe DSL paged MQA logits, TopK, memory-pool,
and cache-transfer surfaces where they match the model contract.
...
| --- | --- |
| `quant_method: "nvfp4_e2m1_ue8m0"` | Selects TensorRT-LLM's FP4 indexer K cache. |
...
The adapter selects TensorRT-LLM's FP4 indexer K cache for NVFP4 indexer model
cards and enables the CuTe DSL FP4 paged MQA logits and TopK surfaces by
default when the model card provides indexer overrides.
...
validates that it uses the FP4 indexer K cache. Its current execution target is
TensorRT-LLM's native FP4 DSA path with IndexCache reuse; a dedicated HISA block
selector should be implemented against the existing TensorRT-LLM FP4 paged MQA
logits and TopK surfaces before treating HISA as performance-equivalent to the
optimization-playground kernel.
...
The implementation adapts TensorRT-LLM's native FP4 DSA/indexer cache machinery
rather than wholesale-porting optimization-playground kernels. Gated Attention
and GatedNorm are unique Blaise model semantics and are ported directly.
