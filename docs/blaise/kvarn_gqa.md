...
- `tensorrt_llm/_torch/attention_backend/kvarn_gqa_attention.py` is the first
  ...
  device int64 storage before store/read; BDR restore uses that device table,
  `physical_commit_gen`, and `restored_gen` plus `torch.unique` on stale
  physical block ids, so reference dequant cost follows churn rather than the
  full working set. Receiver-side restore also rebuilds physical generation from
