...
| HF deployability/default | **Implemented, fail-closed by default** | HF can request/default `kvarn_k2v2_g128` via top-level `kv_cache_dtype`, `quantization_config.kvarn.gqa`, or explicit SMC `draft_kv_cache_dtype="kvarn_k2v2_g128"`; omitted enabled `gqa.dtype` means the production default, while an explicit invalid `kvarn_*` top-level dtype or non-GQA-KVarN nested `gqa.dtype` fails config validation instead of being silently rewritten. Startup rejects GQA KVarN unless the fused store/decode/sparse-decode/dequant ops are registered and the readiness gate passes. |
