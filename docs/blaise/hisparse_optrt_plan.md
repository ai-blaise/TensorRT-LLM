...
- optimize `sparse_mla_decode_kvarn_hot` beyond the direct per-row/head kernel
  ...
  hkv, 288]` plus scale layout as a compatibility backend. Confucius produced
  the initial bounded VM worktree candidate at
  `a4-us-001-rl9:/home/spencer/work/op-trt-hisparse-splitprod-wt` on branch
  `op-trt-hisparse-splitprod`. The June 13 continuation rebased that candidate
  over `origin/op-trt-hisparse` into
  `/home/spencer/work/op-trt-hisparse-splitprod-rebased` on branch
  `op-trt-hisparse-splitprod-rebased`; the equivalent GitHub branch is
  `origin/op-trt-hisparse-splitprod-rebased` at `2fa724252`. That guarded
  branch registers `trtllm::sparse_mla_decode_kvarn_hot_split`, reuses only the
  FlashMLA sparse scheduler metadata, keeps packed `kvarn_k2v2` BDR producer
  loads and explicit sink/tail resident reads, and validates split output/LSE
  parity against the current direct KVarN-hot op for committed hot records,
  resident sink, resident tail, padding-only, invalid upstream row, stale hot
  slot, and stale layer cases. It also fixes the concrete audit blockers that
  could be closed without promoting it to production: the KVarN-specific combine
  no longer has a fixed `splitScale[128]` shared-memory ceiling and computes
  split softmax scales on demand, the fake op metadata row count mirrors the
  B200 scheduler heuristic for concrete dimensions, fake/export symbolic
  dimensions conservatively reserve the maximum 4096 metadata rows, and the
  split launcher inherits the resident sink/tail dtype fail-closed guard. A
  targeted object proof against the cached proof image compiled
  ...
  for split combine. The branch remains promising but not merge-ready for the
  main production path: scheduler/split sizing still needs runtime parity for
