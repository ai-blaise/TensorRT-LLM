...
elementwise+quant, and the DSA Indexer (which is ~50–74 % of TPOT) — rather
than chasing the bandwidth roofline.
...
| 11b | KVarN GQA: SMC-SD generic KV path | [kvarn_gqa.md](kvarn_gqa.md) | safe scaffolding + opt-in reference backend | fail-closed by default |
...
1. [topology_deploy.md](topology_deploy.md) — *where* decode runs (DP2/TP4) and
   ...
   optimizes against.
2. [indexer.md](indexer.md) — the **biggest lever** (Indexer is 50–74 % of
   TPOT). Six composable wins on the DSA Indexer.
...
| **KVarN GQA (11b)** | not Indexer K | generic GQA KV path; reference only behind env opt-in | independent | separate from MLA KVarN | independent | packed-page + side-state proof pending | SMC-SD target path |
